...
def ICLogisticRegression(data, lr, delta):
    ...
    while delta_g > delta:
        ...
        for item in data:
            k = parameters[0] + item[0]*parameters[1] + \
                item[1]*parameters[2] + item[2]*parameters[3]
    ...
    for line in lines:
        ...
        for i in line:
            if i == "*":
                ...
                item.append(i)
        k = parameters[0] + int(item[0])*parameters[1] + int(item[1])*parameters[2] + int(item[2])*parameters[3]
        value = (1 / (1 + math.exp(-k)))
        if value >= 0.5:
            if int(item[3]) != 1:
                print(item)
